...
public class SignCodeHandler extends TextWebSocketHandler{
    ...
    public void handleTextMessage(WebSocketSession session, TextMessage message) {
        ...
        WebSocketSession session1 = sendClient.getSocketSession();
        Map<String,Object> map = session1.getAttributes();
        String code = (String)map.get("code");
        //这里有一个和安卓端传来授权码校验的过程
        try {
            session1.sendMessage(new TextMessage("status"+200+"\n"+"message"+"success"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
